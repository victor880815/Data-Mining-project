...
Original file is located at
    https://colab.research.google.com/drive/11KT1ZK9ropmtQ1VpBW9IPCwvUSpc-cRm
"""
...
df = pd.read_csv('/content/drive/MyDrive/KaggleV2-May-2016 - Copy(100k).csv')
df
...
df.drop(columns=['PatientId', 'AppointmentID', 'ScheduledDay', 'AppointmentDay', 'Neighbourhood', 'No-show'],axis=1,inplace=True)
df
...
features = normalizer.fit_transform(df[['Age']].T.values)
df['Age'] = features.T
df
...
noShow_info = df.drop('Show',axis=1)
for column in noShow_info:
  ig = info_gain.info_gain(df[column], df['Show'])
  igr = info_gain.info_gain_ratio(df[column], df['Show'])
...
class RNN(nn.Module):
  def __init__(self):
    ...
    ### New layers:
    self.lstm = nn.LSTM(11 ,8, 1, batch_first = True)
